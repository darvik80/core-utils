...
namespace em {
    ...
    void Timer::scheduleOnce(const Timer::Duration &duration, const Timer::Handler &fn) {
        ...
        _timer->async_wait([fn](const boost::system::error_code &ec) {
            if (!ec) {
                ...
                timer::log::warning("scheduleOnce canceled: {}", ec.what());
            }
        });
    }
        ...
        auto fnSelf = [timer = _timer, duration, fn](auto &selfRef) -> void {
            ...
            timer->async_wait([fn, selfRef](const boost::system::error_code &ec) {
                if (!ec) {
                    ...
                    timer::log::warning("scheduleAtFixedRate canceled: {}", ec.what());
                }
            });
        };
    ...
    void Timer::scheduleWithFixedDelay(const Timer::Duration &duration, const Timer::Handler &fn) {
        ...
        _timer->expires_from_now(duration);
        _timer->async_wait([fn, duration, self = this](const boost::system::error_code &ec) {
            if (!ec) {
                fn();
                self->scheduleWithFixedDelay(duration, fn);
            } else {
                timer::log::warning("scheduleWithFixedDelay canceled: {}", ec.what());
            }
        });
    }
